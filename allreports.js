...
/* ── Visibility score: likes drive ranking ── */
function visibilityScore(r) {
  return (r.likes || 0);
}

/* ── Compute max likes across reports for bar scaling ── */
function maxLikes(reports) {
  return Math.max(1, ...reports.map(r => r.likes || 0));
}
...
function buildCard(r, maxL) {
  ...
  const fillPct  = Math.min(100, Math.round((likes / maxL) * 100));
  const isHot    = likes > 0 && fillPct >= 60;
  ...
  /* Image */
  const imgSection = r.image_url
    ? `<div class="card-img"><img src="${r.image_url}" alt="Issue photo"/></div>`
  ...
  const adminStrip = privUser ? `
    <div class="admin-controls visible">
      ...
      <button class="btn-change-status" onclick="openModal('${r.id}', '${(r.category||'Report').replace(/'/g,"\\'")}')">
  ...
  return `
    <div class="report-card" id="card-${r.id}">
      ...
      <div class="card-body">
        ...
        <div class="card-meta">
          <span class="card-date"><i class="fas fa-calendar-days" style="margin-right:4px;opacity:.5"></i>${r.date || '—'}</span>
        </div>
      </div>
      <div class="card-footer">
        <button
          ...
          <span id="likeCount-${r.id}">${likes}</span> Same issue
        </button>
...
function renderGrid(reports) {
  ...
  /* Sort descending by visibility (likes) */
...
async function toggleLike(id) {
  ...
  if (countEl) countEl.textContent = report.likes;
...
async function setStatus(newStatus) {
  ...
  const report = allReports.find(r => r.id == activeModalId);
  if (!report) return;
